...
export default function Dashboard() {
  const { can } = useAuth();
  const showFinancial = can('dashboard.financial');
  ...
  const stats = allStats.filter((s) => showFinancial || !s.financial);
    ...
    <div className="p-8">
      <header className="mb-8 surface p-6 border-l-4 border-l-brand-500">
        ...
        <p className="text-slate-600 mt-2 font-medium">
          {showFinancial ? 'Daily operations at a glance' : 'Service overview'}
        </p>
      </header>

      <div className={`grid grid-cols-1 gap-5 mb-8 ${stats.length > 1 ? 'sm:grid-cols-2 xl:grid-cols-4' : 'max-w-sm'}`}>
        {stats.map(({ label, value, icon: Icon, card, iconBg }) => (
          <div
            key={label}
            className={`${card} rounded-2xl p-5 text-white shadow-card hover:scale-[1.02] transition-transform`}
          >
      ...
      <div className="grid lg:grid-cols-2 gap-6">
            ...
            {data?.topItems?.length ? (
              <ul className="space-y-3">
                {data.topItems.map((item, i) => (
                  <li
                    key={item.name}
                    className="flex items-center justify-between p-2 rounded-xl hover:bg-violet-50 transition"
                  >
        ...
        <div className={`card p-6 border-t-4 border-t-violet-500 ${!showFinancial ? 'lg:col-span-2' : ''}`}>
          <h2 className="font-bold text-ink-900 mb-4 flex items-center gap-2">
